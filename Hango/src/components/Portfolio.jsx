...
const Portfolio = () => {
    ...
    // 1. Start the progress bar at 100%
    setProgress(100);
    ...
    const tick = (now) => {
        ...
        // 2. Subtract the elapsed percentage from 100, stopping at 0
        setProgress(
        Math.max(0, 100 - (elapsed / PORTFOLIO_CAROUSEL_INTERVAL) * 100),
        );
          ...
          >
            ...
            >
              {projects.map((project, index) => {
                ...
                const tx =
                  direction *
                  (distance === 1 ? 82 : distance === 2 ? 145 : 190); // % of own width
                const tz = isActive ? 40 : distance === 1 ? -220 : -430; // depth in px (negative = further back)
                const rotY =
                  direction *
                  (distance === 1 ? -31 : distance === 2 ? -42 : -48); // degrees
