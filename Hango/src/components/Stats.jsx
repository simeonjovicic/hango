...
const Stats = () => {
    ...
    return (
        <section ref={containerRef} className="py-10 md:py-16 bg-white w-full">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-6">
                    {stats.map((stat, index) => (
                        <div
                            ...
                            className="group flex flex-col items-center text-center p-4 sm:p-6 rounded-2xl bg-[#f5f5f7] border border-gray-200 transition-all duration-300 hover:border-red-400/70 hover:shadow-[0_0_0_1px_rgba(239,68,68,0.45),0_0_20px_rgba(239,68,68,0.22)]"
                        >
                            <span className="text-2xl sm:text-3xl mb-1.5 sm:mb-2">{stat.icon}</span>
                            <p className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-gray-900 font-roboto tabular-nums">
                                <AnimatedCounter target={stat.value} suffix={stat.suffix} started={countersStarted} />
                            </p>
                            <p className="mt-1.5 sm:mt-2 text-xs sm:text-sm md:text-base text-gray-500 font-inter leading-snug">{stat.label}</p>
                        </div>
                    ))}
                </div>
            </div>
